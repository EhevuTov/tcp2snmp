...
class trap {
  ...
  public:
    ...
    void pduPackAdmin(trap::pdu* data);
	int sendTrap(trap::pdu* data, unsigned long EventID, int LinkID, unsigned int ProcessorID,  unsigned int PortID, unsigned long Direction, unsigned int  MessageType, char TextBuffer[255], int TextLength);
	int sendTrap(trap::pdu* data);
};